...
void draw_line(roger, color, line)
Robot* roger;
int color;
Line line;
{
  int j;
  int request_num = roger->drawing_request.line_number;
  printf("Drawing Line request num: %d %d %lf %lf %lf %lf\n",request_num, color, line.start_x, line.start_y, line.stop_x, line.stop_y);

  if (request_num < NMAX_LINES) {
    roger->drawing_request.line_args[request_num].color = color;

    roger->drawing_request.line_args[request_num].line.start_x = line.start_x;
    roger->drawing_request.line_args[request_num].line.start_y = line.start_y;
    roger->drawing_request.line_args[request_num].line.stop_x = line.stop_x;
    roger->drawing_request.line_args[request_num].line.stop_y = line.stop_y;

    roger->drawing_request.line_args[request_num].line.time = line.time;
    roger->drawing_request.line_number++;
  }
}

void draw_string(roger, color, x, y, text)
Robot* roger;
int color;
double x;
double y;
char* text;
{
  int request_num = roger->drawing_request.text_number;

  if (request_num < NMAX_TEXT) {
    roger->drawing_request.text_args[request_num].color = color;

    roger->drawing_request.text_args[request_num].x = x;
    roger->drawing_request.text_args[request_num].y = y;
    strcpy(roger->drawing_request.text_args[request_num].text, text);
    roger->drawing_request.text_number++;
  }
}

void draw_circle(roger, color, x, y, r, fill)
Robot* roger;
int color;
double x;
double y;
...
int fill;
{
  int request_num = roger->drawing_request.circle_number;
  // printf("Drawing circle request %d %lf %lf %lf %d\n", color, x, y, r, fill);
  if (request_num < NMAX_CIRCLE) {
    roger->drawing_request.circle_args[request_num].color = color;

    roger->drawing_request.circle_args[request_num].x = x;
    roger->drawing_request.circle_args[request_num].y = y;
    roger->drawing_request.circle_args[request_num].radius = r;
    roger->drawing_request.circle_args[request_num].fill = fill;
...
Robot * roger;
{
  ...
  if (roger->enter_param_event) {
    switch(roger->control_mode) {
      ...
      case PROJECT12: project12_enter_params(); break;
      default:
  ...
  switch(roger->control_mode) {
     ...
     case PROJECT11:
       ...
       break;
     case PROJECT12:
       project12_control(roger, roger->simtime);
       project12_visualize(roger);
       break;
     default:
       break;
  }
