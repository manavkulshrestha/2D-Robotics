...
/**************************************************************/
/*     Students need to define the below whenever they        */
/*     build composite skills using primative actions         */
/*           the values below are default values              */
/*     (Not used when building primatives/sensors)            */
#define NUM_ACTIONS 1						// 2
#define NSTATES  3    // 3^(NUM_ACTIONS) // 9
...
double proj_12_q_table[NSTATES][NUM_ACTIONS] = {0.0};
/**************************************************************/
...
// ActionParams params;
// {
// 	int i, state, return_state, internal_state[NUM_ACTIONS]; 
// 	// double search_errors[NUM_DOF], track_errors[NUM_DOF];
// 	double action_errors[NUM_ACTIONS][NUM_DOF];
// 	static int initialized = 0;
// 	int selected_action;

// 	// arrays to hold the action and reward functions (rewards are still tracked for debugging)
// 	static int (*actions[NUM_ACTIONS])(Robot* roger, double errors[NUM_DOF], double time, ActionParams params);
...
// 		LoadLearnedQTable(..., proj_X_q_table, NSTATES, NUM_ACTIONS, Q_TABLE_FILE);
...
// 	selected_action = GetActionGreedy(state, proj_..._q_table, NUM_ACTIONS);
...
// int reset_ball_X(Robot * roger, double time, int state, int internal_state[NUM_ACTIONS], int previous_action, int counter, int episode_len) {
// 	double new_x, new_y;
...
double time;
{
	...
	// int (*actions[NUM_ACTIONS])(Robot* roger, double errors[NUM_DOF], double time, ActionParams params);
	...
	// Q_Learning(roger, time, alpha, gamma, actions, rewards, default_reward, NUM_ACTIONS, NSTATES, 
    ...
    // if (policy_loaded == 0) {
	// 	LoadLearnedQTable(q_table_num, proj_X_q_table, NSTATES, NUM_ACTIONS, Q_TABLE_FILE);
	// 	policy_loaded = 1;
	// }

    // // exploit the learned policy
	// Run_Policy(roger, time, actions, rewards, default_reward, NUM_ACTIONS, NSTATES, reward_num, proj_X_q_table);
	...
	// static int action_calls[NUM_ACTIONS];
	...
	// //     create the sub-folder first!
	// generatePerformanceData(roger, time, actions, rewards, default_reward, NUM_ACTIONS, NSTATES, reward_num, proj_X_q_table,
