...
#define REMOVE_DIST 0.1
#define NUM_DRAW_VERTS 50
...
/* world to pixmap panels (drawing) transformations: Development environment */
...
/* pixmap to world: Development Environment */
...
#define SAT_COLOR    RED
#define ARM_COLOR    BLUE
#define EYE_COLOR    DARKGREEN
#define GOAL_COLOR   DARKGREEN
...
// pushes the toubox outside of the drawable canvas
// Toy Box space
...
// a composite of N elastic spheres at the vertices of a regular polygon
// NOTE: not including an array for the distances to the elestic spheres, as
//     we can compute those at runtime as needed
typedef struct _toyobject {
  int id;                       // so far CIRCLE || TRIANGLE but any N can be used
  int N;                        // number of tiling spheres for this object
  double vertices[50][2];      // 2D array - up to 100 vertices [1]-> the distance from the body frame to the center [2]-> theta value
  double radii[50];          // element i is the radius of the elastic sphere located at vertex i
  int color;               // color of the object
  double image_radius;     // radius used to make images
  double mass;                  // toal mass of the entire polyball
  double moi;                   // moment of inertia ( total_mass * SQR(radius) )
  double position[3];           // position (x,y,theta) of the object
  double velocity[3];           // velocity of the object
  double net_extForce[3];       // from collisions with other objects
  int N_draw;                   // number of verticies used to draw the object mask
  double draw_verts[50][2];
} ToyObject;
...
void project12_enter_params(), project12_reset(),
project12_visualize(), project12_control();
